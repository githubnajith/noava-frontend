...
import Category_card_ui from "@/ui/2.common-components/3.products/1.Category-card-ui";
// ------------------------------------------------------------
interface Params {
  params: {
    maincat_id: string;
  ...
  return (
    <>
      <main className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-8 px-4 py-2">
        {/* custom skin */}
        {customSkin_data?.inputData && <Category_card_ui alt={customSkin_data?.image?.imageName} heading={customSkin_data?.inputData?.name} link={`/customSkinProductDetail/${params?.maincat_id}`} src={customSkin_data?.image?.imageUrl} />}
        {/* --------------------- */}
        {subcat_data &&
          Object.keys(subcat_data).map((key) => {
            return <Category_card_ui key={key} src={subcat_data[key].image.imageUrl} alt={subcat_data[key].image.imageName} link={`/skin-subcategory/${params?.maincat_id}/${key}`} heading={subcat_data[key]?.inputData?.name} />;
          })}
        {/* --------------------- */}
      </main>
    </>
